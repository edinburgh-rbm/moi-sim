...
trait ChangeHelper {
   ...
   */
  def intersectAndStore[T <: StormState[T]](m: collection.mutable.Map[Double, T],
					    state: T, 
					    init: collection.mutable.Map[Int, StormField[_]],
					    chgs: List[StormChange],
					    t: Double, dt: Double): 
  Option[Tuple3[Double, Double, List[StormChange]]] = {
    // Slice up the time frame into interesting parts
    val sliced = slices(chgs)
    //println(s"Slices are: ${sliced.keys}, made from changes: $chgs")
   ...
   * the simulator can decide what to do with them. 
   */
  def intersect(state: StormState[_ <: StormState[_]],
		init: collection.mutable.Map[Int, StormField[_]],
		chgs: List[StormChange],
		t: Double, dt: Double): 
  Option[Tuple3[Double, Double, List[StormChange]]] = {
   ...
   * if the merge fails. 
   */
  def tryMerge(init: collection.mutable.Map[Int, StormField[_]],
	       chgs: List[StormChange],
	       t: Double, tEnd: Double):
  Option[List[StormChange]] = {
    ...
    for { chg <- chgs
	 fieldChg <- chg.chg } {
      ...
      if (!init(fieldChg._1).merge(fieldChg._2, chg.dt, tEnd - t)) {
	violatedFields = fieldChg._1 :: violatedFields
      }
